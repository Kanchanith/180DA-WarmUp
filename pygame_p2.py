...
pygame.display.set_caption("P2 Rock Paper Scissors")
...
def on_connect(player2, userdata, flags, rc):
    print("Connection returned result: " + str(rc))
    player2.subscribe("PROMPT", qos=1)
    player2.subscribe("RESULT", qos=1)
    player2.subscribe("SCORE", qos=1)

def on_disconnect(player2, userdata, rc):
    if rc!=0:
        print("Unexpected Disconnect")
    else:
        print("Expected Disconnect")

def on_message(player2, userdata, message):
...
player2.on_connect = on_connect
player2.on_disconnect = on_disconnect
player2.on_message = on_message

player2.connect_async("mqtt.eclipseprojects.io")

player2.loop_start()

running = True
while running:
    screen.fill((230, 204, 188))

    if rock_button.draw(screen):
        print("rock")
        player2.publish("PLAYER2", "r", qos=1)
    elif paper_button.draw(screen):
        print("paper")
        player2.publish("PLAYER2", "p", qos=1)
    elif scissors_button.draw(screen):
        print("scissors")
        player2.publish("PLAYER2", "s", qos=1)
